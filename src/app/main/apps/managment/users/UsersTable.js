...
function UsersTable(props) {
    ...
    useEffect(() => {
        setData(searchText.length === 0 ? users : _.filter(users, item => item.email.toLowerCase().includes(searchText.toLowerCase())))
    }, [users, searchText]);
    ...
    function handleClick(item) {
        props.history.push('/apps/managment/users/' + item.id + '/' + item.handle);
    }
