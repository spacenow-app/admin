...
import SpacenowNavHorizontalCollapse from './SpacenowNavHorizontalCollapse';
...
function SpacenowNavHorizontalGroup(props) {
                                            ...
                                            <ul className={clsx(classes.children, "pl-0")}>
                                                {
                                                    item.children.map((item) => (
                                                        <React.Fragment key={item.id}>
                                                            ...
                                                            {item.type === 'collapse' && (
                                                                <SpacenowNavHorizontalCollapse item={item} nestedLevel={nestedLevel} dense={dense} />
                                                            )}

                                                            {item.type === 'item' && (
                                                                <SpacenowNavHorizontalItem item={item} nestedLevel={nestedLevel} dense={dense} />
                                                            )}

                                                            {item.type === 'link' && (
                                                                <SpacenowNavHorizontalLink item={item} nestedLevel={nestedLevel} dense={dense} />
                                                            )}
                                                        </React.Fragment>
                                                    ))
                                                }
...
SpacenowNavHorizontalGroup.propTypes = {
...
SpacenowNavHorizontalGroup.defaultProps = {};

const NavHorizontalGroup = withRouter(React.memo(SpacenowNavHorizontalGroup));
