...
import SpacenowNavVerticalGroup from './SpacenowNavVerticalGroup';
import SpacenowNavVerticalItem from './SpacenowNavVerticalItem';
import SpacenowNavBadge from './../SpacenowNavBadge';
import SpacenowNavVerticalLink from './SpacenowNavVerticalLink';
...
function SpacenowNavVerticalCollapse(props) {
    ...
    return (
        <ul className={clsx(classes.root, open && "open")}>
            ...
            >
                ...
                {item.badge && (
                    <SpacenowNavBadge className="mr-4" badge={item.badge} />
                )}
                ...
                <Collapse in={open} className="collapse-children">
                    {
                        item.children.map((item) => (
                            ...
                            <React.Fragment key={item.id}>
                                ...
                                {item.type === 'group' && (
                                    <SpacenowNavVerticalGroup item={item} nestedLevel={nestedLevel + 1} active={active} />
                                )}
                                ...
                                {item.type === 'item' && (
                                    <SpacenowNavVerticalItem item={item} nestedLevel={nestedLevel + 1} active={active} />
                                )}

                                {item.type === 'link' && (
                                    <SpacenowNavVerticalLink item={item} nestedLevel={nestedLevel + 1} active={active} />
                                )}
...
SpacenowNavVerticalCollapse.propTypes = {
...
SpacenowNavVerticalCollapse.defaultProps = {};

const NavVerticalCollapse = withRouter(React.memo(SpacenowNavVerticalCollapse));
