...
function SpacenowSearch(props)
{
    ...
    useEffect(() => {
        ...
        {
            return SpacenowUtils.hasPermission(item.auth, userRole)
        }
        ...
        function setNavigation()
        {
            dispatch({
                type : "setNavigation",
                value: SpacenowUtils.getFlatNavigation(navigation).filter(item => itemAuthAllowed(item))
            });
        }
...
SpacenowSearch.propTypes = {};
SpacenowSearch.defaultProps = {
    trigger: (<IconButton className="w-64 h-64"><Icon>search</Icon></IconButton>),
    variant: 'full'// basic, full
};

export default withRouter(React.memo(SpacenowSearch));
