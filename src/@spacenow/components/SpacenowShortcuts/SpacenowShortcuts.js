...
import { SpacenowUtils, SpacenowAnimateGroup } from '@spacenow';
...
function SpacenowShortcuts(props) {
    const dispatch = useDispatch();
    const shortcuts = useSelector(({ auth }) => auth.user.data.shortcuts);
    const navigationData = useSelector(({ spacenow }) => spacenow.navigation);
    ...
    const shortcutItems = shortcuts ? shortcuts.map(id => SpacenowUtils.findById(navigationData, id)) : [];

    useEffect(() => {
        function flattenNavigation() {
            setNavigation(SpacenowUtils.getFlatNavigation(navigationData));
        }
...
SpacenowShortcuts.propTypes = {};
SpacenowShortcuts.defaultProps = {
    variant: "horizontal"
};

export default React.memo(SpacenowShortcuts);
